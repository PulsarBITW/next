...
const TestClient = () => {
  ...
  return (
    <div className={classes.cont}>
      <p>
        router.refresh() - вызывает обновление маршрута - то есть выполняется
        новый запрос на сервер происходит повторный рендеринг компонентов и
        запросов, НО сохраняется state и состояние браузера (например полоса
        прокрутки)
      </p>
      ...
      <p>
        Так же группирование маршрутов позволяет создавать несколько корневых
        layout'ов - для этого необходимо удалить корневой layout самого верхнего
        уровня и добавить свой layout для каждой группы маршрутов. При этом
        начальная страница - "/" должна находиться в какой-то группе маршрутов -
        иначе у него НЕ будет корневого layout'а
      </p>
      <p className={classes.important}>
        Навигация по нескольким КОРНЕВЫМ layout'ам приведет к полной загрузке
        страницы - это применимо ТОЛЬКО к КОРНЕВЫМ layut'ам. Т.е. если у нас у
        всех группы свои корневые layout'ы - тогда переход по разным группам
        вызовет полную перезагрузку страницы
      </p>
      <p>
        *Имя папки группы маршрутов - НЕ имеет значения, так как это имя НЕ
        попадает в url
      </p>
